             ...
             tabPanel(
               ...
               sidebarLayout(
                 div(id = "Sidebar",
                     sidebarPanel(
                       width = 2,
                       # Load auto-load module function
...
server <- function(input, output, session){
  ...
  # Take return value from auto_load module
  data <- fileAutoloadServer("ExpMat", script_path = script_path)
  ...
  #Take return value from auto-load module
  phen <- fileAutoloadServer("traitData", script_path = script_path)

  observeEvent(input$starttrait, {
    if(is.null(phen())) {
      showNotification("Trait data is not loaded properly.", type = "error")
      return()
    }
    if(is.null(exp.ds$table2)) {
      showNotification("Expression data is not loaded properly.", type = "error")
      return()
    }
    # Rest of the code
  })
