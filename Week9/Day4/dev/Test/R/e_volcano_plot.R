...
evolPlotServer <- function(id, data, event_column, fold_change_column, significance_column, significance_threshold, fold_change_threshold) {
  moduleServer(id, function(input, output, session) {
    ...
    df <- reactive({
      ...
      req(event_column(), fold_change_column(), significance_column())  # Ensure columns are specified
      
      # Calculate Direction based on thresholds
      data() %>%
        mutate(
          Direction = case_when(
            !!sym(fold_change_column()) > fold_change_threshold ~ 'Upregulated',
            !!sym(fold_change_column()) < -fold_change_threshold ~ 'Downregulated',
            TRUE ~ 'No Change'
          ),
          Highlighted = !!sym(event_column()) %in% input$highlighted
        ) %>%
        mutate(
          Direction = case_when(
    ...
    output$e_volcano_plot <- renderPlot({
      req(df())  # Ensure df is available before plotting
      EnhancedVolcano(
        toptable = df(),
        lab = df()$Label,
