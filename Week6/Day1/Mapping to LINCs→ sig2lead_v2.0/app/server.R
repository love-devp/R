...
source("/srv/shiny-server/lib/chemmineR_2_option.R")
source("/srv/shiny-server/lib/cluster_fpsim.R")
...
#  output$Go <- eventReactive(input$Go,  
    {
      ...
      withProgress(message = "Running Sig2Lead", value = 0, {
        ...
        source("/srv/shiny-server/lib/GeneKD.R", local = TRUE)
        source("/srv/shiny-server/lib/ConDisconSignatures.R", local = TRUE)
        
        
        lincs_compounds <- read.csv("/srv/shiny-server/files/LINCSCompounds.csv", stringsAsFactors = FALSE)
        #lsm_rows <- 1:41572
      #1. Query iLINCS for user input gene
        if (input$Signature == "Define Target Gene"){
          ...
          source("/srv/shiny-server/lib/GetSMILES.R", local = TRUE)
          ...
          # 
           ...
           display <<- display
          # 
           source("/srv/shiny-server/lib/ChemmineOB.R", local = TRUE)
          # 
          ...
          # If Bypass Clu
          #source("/srv/shiny-server/lib/chemmineR.R", loca
        }  
        else if (input$Signature == "Upload a Signature"){
          source("/srv/shiny-server/lib/UploadSignature.R")
      ...
      #5. Cluster compounds identified through LINCS, generate heatmaps
        # If Bypass Clu
        #source("/srv/shiny-server/lib/chemmineR.R", local = TRUE)
         ...
         if (Bypass == FALSE)      {                     
                      ...
                      if(!(grepl(".sdf", adds$name))){
                       ...
                       # adds_SMI <<- read.SMIset(paste(getwd(), adds$name, sep="/"))
                        ...
                        source("/srv/shiny-server/lib/ColorMap.R", local = TRUE)
                        color_dend()
                      }
                      else if(grepl(".sdf", adds$name)){
                        print("Added compounds in sdf format")
...
#    else{print("Clustering bypassed")
      ...
      #5.2 Get combined score
        ...
        #source("/srv/shiny-server/lib/Combined_Score_rev_no_auto.R")
        # source("/srv/shiny-server/lib/Combined_Score_auto_2.R")
        # source("/srv/shiny-server/lib/Combined_Score_auto_3.R")
        # if (!is.null(input$AddedCompounds)){
...
#         #source("/srv/shiny-server/lib/Centroid.R", local=TRUE)
#         source("/srv/shiny-server/lib/Centroid2.R", local=TRUE)
...
#   )  
# # output Combined Scores
# #  source("/srv/shiny-server/lib/Combined_Score.R")
...
#     source("/srv/shiny-server/lib/SimilaritySearch.R")
#     nci_list <- list()
...
# #     #source("/srv/shiny-server/lib/StitchNetFn3.R")
# #     source("/srv/shiny-server/lib/StitchNetFn7.R")
# #     print("sourced")
...
###############################################################################################################################################
  ...
  observeEvent(input$GetSTITCH, {
    ...
    #source("/srv/shiny-server/lib/StitchNetFn3.R")
    #source("/srv/shiny-server/lib/StitchNetFn9.R")
    source("/srv/shiny-server/lib/StitchNetFn9_updated.R")
    #source("/srv/shiny-server/lib/StitchNetFn9.R")
    print("sourced")
    #stitchNet("sirolimus", "mtor")
    SelectedCluster <- input$ClusterNumber
