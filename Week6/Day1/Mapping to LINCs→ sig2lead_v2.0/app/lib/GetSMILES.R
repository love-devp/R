...
GetSMILES<- function()
{
  Compounds<<-read.csv(file="/srv/shiny-server/files/LINCSCompounds.csv", stringsAsFactors = FALSE)
  ...
  for (i in 1:length(con_comma))
  {
    ...
    if(input$ILINCS == "Current"){
      #if(ilincs == "Current"){
      #if(input$ILINCS == "Current"){ 
      #if (length(con_comma[[i]][[12]])!=0){
      if (length(con_comma[[i]][[13]])!=0){
        #rows_to_add <- which(Compounds$SM_LINCS_ID %in% con_comma[[i]][[12]])
        #i <- 2
        rows_to_add <- which(Compounds$SM_LINCS_ID %in% con_comma[[i]][[13]])
        compound_rows[[i]] <- rows_to_add
      }
      else {compound_rows[[i]] <- 0}
    }
