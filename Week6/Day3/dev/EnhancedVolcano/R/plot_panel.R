...
plotServer <- function(id, data, event_column, fold_change_column, significance_column, expression_column, fold_change_threshold, significance_threshold) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    volcano_plot = reactive({
      ...
      # Explicitly convert any other required inputs to lists if necessary
      highlighted_events <- input$highlighted
      if (!is.null(highlighted_events)) {
        highlighted_events <- as.list(highlighted_events)
      }
