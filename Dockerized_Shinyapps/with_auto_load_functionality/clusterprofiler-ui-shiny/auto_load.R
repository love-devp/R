...
fileAutoloadServer <- function(id, script_path) {
  moduleServer(id, function(input, output, session) {
    ...
    input_files <- reactive({
      ...
      for (f in all_files) {
        if (substr(f, 1, 3) != 'run' & str_detect(f, 'inputs')) {  # Check conditions for the files
    ...
    observeEvent(input_files(), once=TRUE, ignoreNULL = FALSE, {
    ...
    selected_file_contents <- reactive({
      if (input$input_source == "Automatic") {
        file_path <- paste0(script_path(), input$automatic_data)  # Get the selected automatic file path
      } 
      ...
      if (!is.null(file_path) && file_path != script_path() && file.exists(file_path)) {
