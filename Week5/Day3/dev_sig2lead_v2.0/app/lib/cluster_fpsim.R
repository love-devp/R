fpsim_cluster <- function(fpset_added, lsm_rows, NumberCmpds){
  ...
  NumberCmpds <- as.integer(NumberCmpds)
  if (nrow(fpset_input) < NumberCmpds){
    fpset_input <- fpset_input
  }
  else if ((nrow(fpset_input) >= NumberCmpds)){
    fpset_input <- fpset_input[1:NumberCmpds,]
    print(paste("Number of rows in fpset_input", nrow(fpset_input)))
  }
 # else if (((nrow(fpset_input) >= 500) & (all_cmpd==1))){
#    fpset_input <- fpset_input
#  }
  
 fpset_input_test <<- fpset_input
...
fpsim_cluster_no_added <- function(lsm_rows, NumberCmpds){
  load("./lincs_fps.RData")
  #fpSim_start_time <- Sys.time()
  #lsm_rows <- 1:100
  lincs <- lincs_fps_2[lsm_rows,]
