  ...
  moduleServer(id, function(input, output, session) {
    ...
    volcano_plot <- reactive({
      ...
      create_volcano_plot(
        ...
        title = NULL,  # You can customize the title
        subtitle = NULL,  # You can customize the subtitle
        caption = NULL,  # You can customize the caption
        pCutoff = significance_threshold(),
        FCcutoff = fold_change_threshold(),
        #pointSize = input$pointSize,
        labSize = input$label_size,
        #col = colors(),  # Use the colors reactive object
        #colAlpha = unlist(alphas()),  # Use the alphas reactive object
        shape = 19,  # Default point shape
        cutoffLineType = 'dashed',  # Equivalent to `linetype=2`
        cutoffLineCol = input$fc_line_color,
        cutoffLineWidth = 0.5,  # Adjust line width if needed
        drawConnectors = TRUE,  # Set to TRUE if you want connectors
        widthConnectors = 0.5,  # Adjust connector width if needed
        ...
        vlineType = 'dashed',  # Line type for vline
        vlineCol = input$fc_line_color,
        vlineWidth = 0.5,  # Line width for vline
        legendPosition = 'none',  # You can set to "right" if you want a legend
        max.overlaps = 50  # To control label overlap behavior
      ))
      } else {
        return(NULL)
      }
